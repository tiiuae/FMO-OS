{
  ...
  cfg = config.services.mydesktopManager.plasma6;
...
in {
  options = {
    services.mydesktopManager.plasma6 = {
      enable = mkOption {
        type = types.bool;
        default = false;
  ...
  config = mkIf cfg.enable {
   ...
   # programs.kde-pim.enable = mkDefault true;
    ...
    programs.chromium = {
      ...
      homepageLocation = "192.168.101.11";
    };
    ...
    #environment.etc."fmo.knsv".source = ./fmo.knsv;
    systemd.services."kde-profile" = {
      script = ''
        mkdir -p /home/ghaf/.config
        cp -R ${./config}/* /home/ghaf/.config/
      '';
      serviceConfig = {
        Type = "oneshot";
        Restart="on-failure";
