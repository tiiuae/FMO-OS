...
# SPDX-License-Identifier: Apache-2.0
{
  ...
  inherit (import "${ghafOS}/lib/launcher.nix" {inherit pkgs lib;}) rmDesktopEntries;
  guivmBaseConfiguration = {
    imports = [
      (import "${ghafOS}/modules/microvm/virtualization/microvm/common/vm-networking.nix" {
        inherit config lib vmName macAddress;
        internalIP = 3;
      })
      # To push logs to central location
      #(import "${ghafOS}/modules/common/logging/client.nix")
      ({
        lib,
        pkgs,
        ...
  ...
  vsockproxy = pkgs.callPackage "${ghafOS}//packages/vsockproxy" {};

  # Importing kernel builder function and building guest_graphics_hardened_kernel
  buildKernel = (import "${ghafOS}/packages/kernel") {inherit config pkgs lib;};
  config_baseline = "${ghafOS}/modules/hardware/x86_64-generic/kernel/configs/ghaf_host_hardened_baseline-x86";
