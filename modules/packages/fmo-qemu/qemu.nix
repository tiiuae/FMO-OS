...
let
  ...
  # FMO: Convert string syntax to bash array syntax which is easier to 
  # add command arguments
  toCommandArray = lib.concatMapStringsSep " " (arg:
    let
      string = toString arg;
    in
      if builtins.match "[[:alnum:],._+:@%/-]+" string == null
      then "\"${builtins.replaceStrings ["'"] ["'\\''"] string}\""
      else string);
  ...
  pci-passthrough-options = pkgs.callPackage ./pci-passthrough-options.nix {inherit pkgs microvmConfig;};
in
lib.warnIf (mem == 2048) ''
...
''
{
  ...
  command = toCommandArray (
    ...
    # PCI passthrough options
    + " $(eval ${pci-passthrough-options}/bin/pci-passthrough-options)";
